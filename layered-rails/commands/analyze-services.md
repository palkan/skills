...
This is a focused complement to `/layered-rails:analyze`. Where the parent command surveys the whole codebase and emits a **brief** service-layer summary (tier, convention strength, top 3 cluster headlines, smell counts), this command produces the **full** report — empirical mirror, per-cluster proposal blocks with contracts and test wins, classification rules, cross-cutting findings. Run `/layered-rails:analyze` first for the wide view; run this command when the brief surfaces something worth resolving.
...
3. **Callback proliferation.** Side effects accumulate as `after_*_commit` callbacks. Run `/layered-rails:analyze-callbacks`.
4. **Concerns become a junk drawer.** Watch concerns crossing 100 LOC or with mixed responsibilities.
5. **Job classes attract scope creep.** Run `/layered-rails:analyze-gods` over `app/jobs/` periodically.
